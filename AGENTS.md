...
All session state lives in `src/context/SessionContext.tsx` via `useReducer` + `captionReducer`. The context wires together 6 hooks and exposes `startSession` / `endSession` / `giveFeedback(choice: 'yes' | 'no')` to components. It also owns the active `displayMode` (`'lecture' | 'group'`) and persists that mode to localStorage.
...
- `MAX_CAPTION_LINES = 150` — caps in-session caption lines (~9–10 min before trimming)
- `MAX_WORDS_PER_LINE = 8` — Deepgram chunks split at this word count
- `DISPLAY_MODE_KEY` — localStorage key for persisting Lecture/Group mode

`src/lib/transcriptExport.ts`:
- `printTranscript(data)` — opens browser print dialog with a mode-aware PDF-friendly document (lecture or group)
- `buildTranscriptText(data)` — returns plain-text transcript string

`src/lib/speakers.ts`:
- `getSpeakerProfile(id: 1-4)` — returns `{ bgColor, textColor }` for Group mode speaker blocks
...
| `useDisplayMode()` | Legacy localStorage helper; current app flow reads `displayMode` from `SessionContext` |
